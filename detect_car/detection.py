...
def load_ground_truth(file):
    ...
    for l in land_marks:
        list1 += [l[0]]
        list2 += l[1:]
    box.append(min(list1))
    box.append(min(list2))
    box.append(max(list1))
    box.append(max(list2))
...
def face_detection(imgList):
    ...
    for imgFile in open(imgList).readlines():  # 对于每个测试图片
        scales = []
        factor = 0.793700526
        img = cv2.imread(imgFile.strip())  # 读取图片
        img_name = imgFile.strip().split('.')[0]
        ground_truth = load_ground_truth(img_name+'.txt')
        min_range = 0
        max_range = 0
        if (img.shape[0] > img.shape[1]):
            min_range = img.shape[1]
            max_range = img.shape[0]
        else:
            min_range = img.shape[0]
            max_range = img.shape[1]

        delim = 1200 / max_range
        if (delim == 1):
            scales.append(1)
        elif (delim > 1):
            scales.append(delim)

        #scales.append(pow(factor, time))#(pow(factor, time)
        ...
        for scale in scales:
            ...
            # cv2.imwrite("tmp2.jpg", scale_img)
            print 'resize:', scale_img.shape[0], scale_img.shape[1]
            # modify the full_conv prototxt.
            prototxt = open(model_define_fc, 'r')
            new_line = ""
            for i, line in enumerate(prototxt):
                if i == 5:
                    new_line += "input_dim: " + str(scale_img.shape[0]) + "\n"  #
                elif i == 6:
                    new_line += "input_dim: " + str(scale_img.shape[1]) + "\n"
                else:
                    new_line += line
            ...
            out = net_full_conv.forward_all(data=np.asarray([transformer.preprocess('data', scale_img)]))
            # print out['prob'][0, 1]
